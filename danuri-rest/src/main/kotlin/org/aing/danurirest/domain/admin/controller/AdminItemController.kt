...
class AdminItemController(
    ...
    @PostMapping
    fun createItem(@Valid @RequestBody request: ItemRequest): ResponseEntity<ItemResponse> =
    ...
    fun deleteItem(@PathVariable itemId: UUID): ResponseEntity<Unit> =
        itemManagementUsecase.deleteItem(itemId).run {
            ResponseEntity.noContent().build()
        }
    
    @GetMapping("/{itemId}")
    fun getItem(@PathVariable itemId: UUID): ResponseEntity<ItemResponse> =
